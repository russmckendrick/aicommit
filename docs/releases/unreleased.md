...
- Refactored oversized Rust modules into folderized internal submodules such as `src/git/`, `src/config/`, `src/prompt/`, `src/generator/`, `src/history_store/`, `src/ai/command/`, `src/commands/commit/`, and `src/commands/history/` while preserving the existing CLI behavior and public module paths.
